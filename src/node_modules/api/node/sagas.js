...
import { formatSchemaErrors, ClientSettings, HomeNotConnectedError, NodeApi, NodeApiError, NodeError } from "api";
...
function* callHome(options) {
    ...
    const location = urlWithParameters(options.location, {"token": token});
    return yield call(callApi, {
        ...options,
        location,
...
export function* updateRegisteredName(name, mnemonic) {
    const location = yield call(authorized, "/registered-name");
...
export function* getPosting(id, withSource = false) {
    ...
    const location = yield call(authorized, urlWithParameters(`/postings/${id}`, {include}));
...
export function* putPosting(id, postingText) {
    ...
    return yield call(callNode, {location, method: "PUT", body: postingText, schema: NodeApi.PostingInfo});
}
...
export function* getNodeSettings() {
    return yield call(callHome, {location: "/settings/node", schema: NodeApi.SettingInfoArray});
}

export function* getNodeSettingsMetadata() {
    return yield call(callHome, {location: "/settings/node/metadata", schema: NodeApi.SettingMetaInfoArray});
}
...
export function* putSettings(settings) {
    return yield call(callHome, {location: "/settings", method: "PUT", body: settings, schema: NodeApi.Result});
}
