...
import { POSTING_SET } from "state/posting/actions";
...
export default (state = initialState, action) => {
    switch (action.type) {
        ...
        case TIMELINE_FUTURE_SLICE_SET:
            ...
            action.payload.postings.forEach(p => istate.set([p.id, "posting"], safeguard(p)));
            return istate.value();

        case POSTING_SET:
            const posting = action.payload.posting;
            return immutable(state).set([posting.id, "posting"], safeguard(posting)).value();
