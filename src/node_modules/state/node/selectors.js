...
export function isNodeAdmin(state) {
    const isHome = state.home.root.api === state.node.root.api;
    const permissions = getNodePermissions(state);
    return isHome && permissions != null && permissions.includes("admin");
}
        ...
        switch (r) {
            case "owner":
                if (state.home.owner.name === object.ownerName
                    && state.home.owner.generation === object.ownerGeneration) {
