...
class SettingsSheetOther extends React.PureComponent {
    ...
    settingTitle(name) {
        const meta = this.props.nodeMeta.get(name);
        ...
        return (
            <Form>
                {[...nodeValues.keys()].sort().map(name =>
                    <InputField key={name} name={toFieldName(name)} title={this.settingTitle(name)} />
                )}
...
const settingsSheetOtherLogic = {
    ...
    mapPropsToValues(props) {
        let values = {};
        props.nodeValues.forEach((value, name) => values[toFieldName(name)] = value || "");
        return values;
    },
