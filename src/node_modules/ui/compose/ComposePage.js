...
const PublishAtImpl = ({formik}) => (
    formik.values.publishAtCustomized &&
        <DateTimeField title="Publish at" name="publishAt" horizontal={true} groupClassName="pl-4" col="col-md-3" />
);

const PublishAt = connectFormik(PublishAtImpl);

const SubmitButtonImpl = ({disabled, update, formik}) => {
    disabled = disabled || formik.values["body"].trim().length === 0;
    const title = !update ? "POST" : "UPDATE";
    return <Button variant="primary" block type="submit" disabled={disabled}>{title}</Button>
};
...
class ComposePage extends React.PureComponent {
    ...
    render() {
        const {loadingFeatures, sourceFormats, loadingPosting, postingId, beingPosted, goToPosting} = this.props;
        ...
        return (
            <Page>
                ...
                <Form>
                    ...
                    <SubmitButton disabled={beingPosted} update={postingId != null} />
...
const composePageLogic = {
    ...
    mapPropsToValues(props) {
        return {
            body: props.body || (props.posting != null ? props.posting.bodySrc : ""),
