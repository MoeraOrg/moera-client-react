...
import { AndroidMediaUploadManager } from "api/android/android-media-upload";
...
describe("AndroidMediaUploadManager", () => {
    let manager: AndroidMediaUploadManager;

    afterEach(() => manager?.dispose());

    test("enables API version 3 with a capability handshake", () => {
        const android = createAndroid();
        manager = new AndroidMediaUploadManager(() => android, async value => value as PrivateMediaFileInfo);
    ...
    test("retains the legacy path when API version 3 is unavailable", () => {
        ...
        manager = new AndroidMediaUploadManager(() => android);
    ...
    test("maps Android selection without exposing a URI", () => {
        const android = createAndroid();
        manager = new AndroidMediaUploadManager(() => android);
        manager.initialize();
    ...
    test("distinguishes Android references in a mixed local selection", () => {
