...
import { AndroidMediaRef, LocalMediaSource } from "api/android/media-source";
...
function createAndroid(apiVersion: number = 3) {
    const android = {
        ...
        startMediaUpload: jest.fn<(json: string) => void>()
    };
...
interface HarnessProps {
    ...
    onSelectedMedia: (files: LocalMediaSource[]) => void;
    onRestoreMedia: (files: AndroidMediaRef[]) => void;
...
function renderUploader(draftId: string | null, mediaIds: readonly string[] = []) {
    ...
    const onSelectedMedia = jest.fn<(files: LocalMediaSource[]) => void>();
    const onRestoreMedia = jest.fn<(files: AndroidMediaRef[]) => void>();
...
describe("useAndroidUploader", () => {
    ...
    test("starts and completes an upload owned by the hook draft", async () => {
        ...
        expect(JSON.parse(android.startMediaUpload.mock.calls[0][0])).toEqual({
            id: source.id,
            draftId: "draft-1",
            downsize: false
        });
    ...
    test("two hook instances restore only their own draft", async () => {
        ...
        const onSelectedMedia1 = jest.fn<(files: LocalMediaSource[]) => void>();
        const onSelectedMedia2 = jest.fn<(files: LocalMediaSource[]) => void>();
        const onRestoreMedia1 = jest.fn<(files: AndroidMediaRef[]) => void>();
        const onRestoreMedia2 = jest.fn<(files: AndroidMediaRef[]) => void>();
        act(() => {
